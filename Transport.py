...
#into the transport dataframe
def addition():
    df=pd.read_csv(r'C:\Users\User\Desktop\transport.csv',encoding=
    'unicode_escape')
    ...
    if c=='yes':
        ...
        company=input("\nenter the transportation company: ")
        pickup=input("\nenter the pick up place: ")
        drop=input("\nenter the drop off place: ")
        HQ=input("\nenter the Headquarters of transport company: ")

        #create new dataframe with the user input data
        #then add it to the existing dataframe
        df1=pd.DataFrame([[cust_id,company,pickup,drop,HQ]],columns=["cust_id","company","pickup","drop_off","HQ"])
        df2=df.append(df1)
        ...
        df2=df2.to_csv(r'C:\Users\User\Desktop\transport.csv',index=False,header=True)
        df=pd.read_csv(r'C:\Users\User\Desktop\transport.csv',encoding='unicode_escape')
        print("\ntable updated\n",df)
...
def deletion():
    ...
    df.to_csv(r'C:\Users\User\Desktop\transport.csv',
...
def modify():
    ...
    print("Row modified!!!\n",df)
    df.to_csv(r'C:\Users\User\Desktop\transport.csv', index=False,
    header=True)
