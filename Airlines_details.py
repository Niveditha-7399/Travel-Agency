...
#into the tp_details dataframe
def addition():
    df=pd.read_csv(r'C:\Users\User\Desktop\al_details.csv',encoding=
    'unicode_escape')
    ...
    if c=='yes':
        ...
        price=input("\nenter the price in dhs: ")
        p_vat=input("\nenter the price with VAT: ")
        
        #create new dataframe with the user input data
        #then add it to the existing dataframe
        df1=pd.DataFrame([[cust_id,dpt,arr,price,p_vat]],columns=["cust_id","dpt_date&time","arr_date&time","price","price with VAT"])
        df2=df.append(df1)
        
        #write the new dataframe to the csv file
        #read it back from the csv file
        #and display it to the user
        df2=df2.to_csv(r'C:\Users\User\Desktop\al_details.csv',index=False,
        header=True)
        df=pd.read_csv(r'C:\Users\User\Desktop\al_details.csv',encoding='unicode_escape')
        print("\ntable updated\n",df)
...
def deletion():
    ...
    df=pd.read_csv (r'C:\Users\User\Desktop\al_details.csv')
    print("\nThis is the existing table:")
    print(df)
    ...
    df.to_csv(r'C:\Users\User\Desktop\al_details.csv',
...
def modify():
    ...
    print("Row modified!!!\n",df)
    df.to_csv(r'C:\Users\User\Desktop\al_details.csv', index=False,
    header=True)
